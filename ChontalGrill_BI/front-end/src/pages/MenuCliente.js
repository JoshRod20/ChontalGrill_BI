...
function Galeria({ rol }) {
  ...
  const searchFilteredMenus = Array.isArray(menus) ? menus.filter((menu) => {
  ...
  const calificacionPromedio = Array.isArray(comentarios) && comentarios.length > 0
    ? comentarios.reduce((acc, comentario) => acc + comentario.Calificacion, 0) / comentarios.length
    : 0;
  const totalOpiniones = Array.isArray(comentarios) ? comentarios.length : 0;
