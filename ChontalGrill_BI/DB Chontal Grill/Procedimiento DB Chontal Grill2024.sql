...
BEGIN
    INSERT INTO metodo_de_pago (Descripcion) VALUES (descripcion_param);
END //
DELIMITER ;
...
CREATE PROCEDURE ActualizarMetodoPago(
    IN id_param INT,
    IN nueva_descripcion_param VARCHAR(100)
)
BEGIN
    UPDATE metodo_de_pago 
    SET Descripcion = nueva_descripcion_param 
    WHERE ID_Metodo_Pago = id_param;
END //
DELIMITER ;
...
BEGIN
    DELETE FROM metodo_de_pago WHERE ID_Metodo_Pago = id_param;
END //
DELIMITER ;
